...
/**
 * Client-only configuration class managing custom canvas, label hex colors, and
 * GUI scale overrides [3]. Config values are built programmatically from the
 * common Color enum to ease future expansion [3].
 */
public class ClientConfig {
	private static final ModConfigSpec.Builder BUILDER = new ModConfigSpec.Builder();

	/**
	 * Forces a specific GUI scale when opening the Machine Inventory Manager screen
	 * [3]. Set to 0 to use standard adaptive auto-scaling [3].
	 */
	...
	/**
	 * Holds the foreground label text config values built programmatically for each
	 * Color [3].
	 */
	...
	static {
		...
		FORCE_GUI_SCALE = BUILDER.comment(
				...
				.translation("sfmflow.configuration.forceGuiScale").defineInRange("forceGuiScale", 0, 0, 8);
		BUILDER.pop();
		...
		for (Color color : Color.values()) {
			...
			// Format current compile-time values to hexadecimal standard #RRGGBB strings
			// [3]
			String defaultBg = String.format("#%06X", color.getDefaultHexColor() & 0xFFFFFF);
			ModConfigSpec.ConfigValue<String> bgVal = BUILDER
					.comment("The background panel and border tint color for " + color.getSerializedName())
			...
			ModConfigSpec.ConfigValue<String> textVal = BUILDER
					.comment("The foreground text and label color for " + color.getSerializedName())
	...
	/**
	 * Defensively parses hexadecimal color strings with integer overflow safety
	 * [3]. Uses Integer.parseUnsignedInt to safely support high-bit Alpha values
	 * [3].
	 *

...
/**
 * Public API base class representing an interactive workspace flowchart component [3].
 * Houses hardcoded coordinates and locked visual dimensions for standard node rendering [3].
 */
public abstract class AbstractFlowComponent {
  ...
  /**
   * Helper record containing core flowchart component properties to streamline Codec usage [3].
   * NBT Purged: Completely removed isOpen, xBeforeOpen, and yBeforeOpen fields to match compact specifications [3].
   */
  public record BaseProperties(
		    ...
		    int z, 
		    String customName, 
		    dta.sfmflow.util.Color colorMask
		) {
		    /**
		     * MapCodec handling the base flowchart component fields.
		     * Fully purged of old toggled coordinates [3].
		     */
		    public static final MapCodec<BaseProperties> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
		        UUIDUtil.CODEC.fieldOf("id").forGetter(BaseProperties::id),
		        ...
		        dta.sfmflow.util.Color.CODEC.optionalFieldOf("colorMask").forGetter(props -> Optional.ofNullable(props.colorMask()))
		    ).apply(instance, (id, x, y, z, customName, colorMaskOpt) -> new BaseProperties(
		        id, x, y, z, customName, colorMaskOpt.orElse(Color.WHITE)
		    )));
		}

  public static final Codec<AbstractFlowComponent> CODEC = FlowComponentType.REGISTRY.byNameCodec().partialDispatch(
      "type",
      component -> com.mojang.serialization.DataResult.success(component.getType()),
      typeEntry -> {
          if (typeEntry == null) {

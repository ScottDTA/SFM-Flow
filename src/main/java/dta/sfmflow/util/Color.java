...
/**
 * Public API enumeration defining the color palette used for visual workspace component masks [3].
 * Stores compile-time default values but routes active color queries through an injectable resolver [3].
 */
public enum Color implements StringRepresentable {
  BLACK("black", ChatFormatting.BLACK, 0x1D1D21, 0xFFFFFF),
  BLUE("blue", ChatFormatting.BLUE, 0x3C44AA, 0xFFFFFF),
  GREEN("green", ChatFormatting.GREEN, 0x5E7C16, 0xFFFFFF),
  CYAN("cyan", ChatFormatting.DARK_AQUA, 0x169C9C, 0xFFFFFF),
  RED("red", ChatFormatting.RED, 0xB02E26, 0xFFFFFF),
  ...
  public static final StreamCodec<ByteBuf, Color> STREAM_CODEC = ByteBufCodecs.idMapper(id -> Color.values()[id], Enum::ordinal);

  // Interface supplier to fetch config values without referencing ClientConfig directly in common code [3]
  private static java.util.function.BiFunction<Color, Boolean, Integer> colorResolver = (color, isText) -> isText ? color.defaultHexTextColor : color.defaultHexColor;
  ...
  /**
   * Configures the active client resolver delegate [3].
   * Called exclusively on the physical client setup phase [3].
   *
   * @param resolver the resolver function mapping Color and text-state to integer [3]
   */

...
/**
 * Server-side configuration definition for the SFM-Flow mod [3]. Houses
 * configurable performance limits and resource restricting bounds, allowing
 * server-safe tuning of search depths, canvas entities, and interval triggers.
...
public class ServerConfig {
	...
	/**
	 * Maximum allowed connected inventories/containers found when scanning via
	 * connected cables [3].
	 */
	public static final ModConfigSpec.IntValue MAX_CONNECTED_INVENTORIES;

	/**
	 * Maximum cable search depth/distance limit for manager network scanning
	 * algorithms [3].
	 */
	public static final ModConfigSpec.IntValue MAX_CABLE_LENGTH;

	/**
	 * Maximum allowed visual flowchart components/nodes placed on the manager
	 * canvas [3].
	 */
	...
	static {
		BUILDER.comment("Performance limits and workspace bounds")
				.translation("sfmflow.configuration.section.sfmflow.server.toml.limits").push("limits");
		...
		MAX_CABLE_LENGTH = BUILDER.comment("Max search depth for cable logic")
				.translation("sfmflow.configuration.maxCableLength").defineInRange("maxCableLength", 128, 1, 512);
		...
		MIN_INTERVAL_TICKS = BUILDER.comment("Minimum ticks allowed for periodic execution triggers")
				.translation("sfmflow.configuration.minIntervalTicks").defineInRange("minIntervalTicks", 4, 1, 1200);

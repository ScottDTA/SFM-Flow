...
public class IntervalTriggerComponent extends AbstractTriggerComponent
 {
  ...
  /**
   * Bulletproof codec mapping the TimeUnit enum values using standard string xmapping [3].
   */
   ...
   * Leverages flat .fieldOf mapping to bypass OptionalFieldCodec validation limits cleanly.
   */
  public static final MapCodec<IntervalTriggerComponent> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
      // 🔥 THE CURE: Use .fieldOf("base") directly on the MapCodec without .codec() wrappers!
  ...
  /**
   * Calculates the total tick count represented by the current interval value and time unit [3].
   * Automatically clamps the resulting value between the server's configured minimum and maximum limits [3].
   *

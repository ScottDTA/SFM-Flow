 ...
 * Delegates mouse interactions directly to a clean helper handler class [3].
 * Features localized scale adjustments, rendering depth protection layers, and side-safe operation [3].
 */
@OnlyIn(Dist.CLIENT)
public class ManagerScreen extends AbstractContainerScreen<ManagerMenu> {
   ...
   */
  private static final ResourceLocation GUI_BG1 = ResourceLocation.fromNamespaceAndPath(SFMFlow.MODID, "textures/gui/background1.png");
  
  /**
   * Resource path locating the right-half background panel asset [3].
   */
  private static final ResourceLocation GUI_BG2 = ResourceLocation.fromNamespaceAndPath(SFMFlow.MODID, "textures/gui/background2.png");

  /**
   * Deferred queue holding components marked for removal to bypass concurrent modification checks during rendering [3].
   */
  ...
  /**
   * Reference pointing to the most recently clicked block entity container card [3].
   */
  ...
  /**
   * Remaining network sync tick-rate throttle ensuring layout changes do not corrupt active drags [3].
   */
   ...
   */
  public ManagerScreen(ManagerMenu menu, net.minecraft.world.entity.player.Inventory playerInventory, Component title) {
	...
	if (forcedScale > 0) {
	  ...
	  if (testWidth >= requiredWidth && testHeight >= requiredHeight) {
		if (currentScale != forcedScale) {
		  ...
		  return; // Exit initialization immediately to let Minecraft refresh the screen state [3].
		}
		scaleApplied = true;
	  } else {
		SFMFlow.LOGGER.warn("[SFM-Flow] Window size is too small for configured FORCE_GUI_SCALE ({}). Falling back to adaptive scaling.", forcedScale);
	  }
	}

	// Pass 2: Adaptive Fallback Scaling (Only run if forced scale was not applied) [3]
	if (!scaleApplied) {
  ...
  public void removed() {
    ...
    // Scale Restoration Shield: Ensure scale is reverted cleanly only if it differs from the original cached setting [3].
    this.mc.tell(() -> {
  ...
  /**
   * Sorts and populates all visual node card containers retrieved from the manager entity [3].
   *
  ...
  public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
    ...
    // Drawn on top of standard components, but beneath hover panels and dropdowns [3]
    ...
    if (hoveredCategoryButton != null) {
      if (this.activeSubmenu == null || this.activeSubmenu.getCategory() != hoveredCategoryButton.getCategory()) {
        this.activeSubmenu = new CategoryHoverSubmenu(
            hoveredCategoryButton.getCategory(), 
            hoveredCategoryButton.getX() + 7, 
            hoveredCategoryButton.getY(), 
            this
        );
      }
    ...
    // Shift menu overlays up to +150.0F to render beautifully on top of shifted card elements [3]
  ...
  /**
   * Recursively resets interaction flags (hovered, tooltip visibility) on the widget tree [3].
   *
  ...
  /**
   * Clears existing visual component widgets and completely rebuilds the layout [3].
   * This is triggered during server delta syncs or layout refreshes [3].
   */
  ...
  /**
   * Schedules a container to be safely removed from the rendering and event handling pipelines on the next render pass [3].
   *
  ...
  /**
   * Package-private getter allowing the mouse coordinator subclass to compile-safely access elements [3].
   *
  ...
  protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
	guiGraphics.drawString(this.font, Component.translatable("gui.sfmflow.commands", getMenu().getCommandCount()), 4, 244, 4210752, false);
  }
  ...
  protected void containerTick() {
    ...
    if (!this.mouseHandler.isDragging() && this.refreshCooldown == 0 && this.getMenu().getManagerBlockEntity().pollNeedsRefresh()) {
      refreshWidgetLayout();
    }
  }

  /**
   * Unpacks a received serverbound node layout delta packet, updating client-side data maps [3].
   *
   * @param packet sync component layout descriptor packet [3]
   */
  public void handleDeltaUpdate(SyncComponentDeltaPacket packet) {
    AbstractFlowComponent localComponent = this.getMenu().getManagerBlockEntity().getFlowComponents().get(packet.componentId());
    ...
    switch (packet.deltaType()) {
      case MOVE -> {
          if (localComponent != null) {
            ...
            for (Renderable r : this.renderables) {
              if (r instanceof FlowWidgetContainer container && container.getComponent().getId().equals(packet.componentId())) {
      ...
      case REMOVE -> {
          ...
          this.getMenu().getManagerBlockEntity().getFlowConnections().removeIf(wire -> 
              wire.getSourceComponentId().equals(packet.componentId()) || wire.getTargetComponentId().equals(packet.componentId())
          );
          refreshWidgetLayout();
      }
     }
  ...
  /**
   * Retrieves the centralized Mouse Input Coordinator managing interaction physics [3].
   *

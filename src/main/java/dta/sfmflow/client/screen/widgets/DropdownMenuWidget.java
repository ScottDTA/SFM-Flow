 ...
 * Houses scrolling logic, viewport scissor bounds, and launches settings modals [3].
 */
@OnlyIn(Dist.CLIENT)
public class DropdownMenuWidget extends AbstractFlowWidget {
    ...
    /**
     * Initializes a Dropdown Menu with a fixed width of 84px and dynamic scrollable height [3].
     *
    ...
    public DropdownMenuWidget(FlowWidgetContainer targetContainer, int mouseX, int mouseY, ManagerScreen parentScreen) {
        ...
        event.addLink(Component.literal("Settings"), () -> {
            NodeSettingsOverlay settingsOverlay = NodeSettingsOverlayFactory.create(parentScreen, targetContainer.getComponent());
            parentScreen.setActiveSettingsOverlay(settingsOverlay);
            parentScreen.setOpenedDropdown(null);
        });

        // Fire public NeoForge event to gather registrations from third-party add-ons [3]
        ...
        int clampedX = Math.max(parentScreen.getLeftPos() + 22, Math.min(mouseX, parentScreen.getLeftPos() + parentScreen.getImageWidth() - this.width - 4));
        int clampedY = Math.max(parentScreen.getTopPos() + 4, Math.min(mouseY, parentScreen.getTopPos() + parentScreen.getImageHeight() - this.height - 4));
        this.setX(clampedX);
        this.setY(clampedY);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.visible && this.active && isMouseOver(mouseX, mouseY)) {
            for (FlowTextLink link : links) {
                // Safeguard clicked checks: Click target only if within the active visible scissor viewport [3]
    ...
    protected void renderComponent(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        // Viewport Scissor to prevent text elements from drawing out-of-bounds on short devices [3]

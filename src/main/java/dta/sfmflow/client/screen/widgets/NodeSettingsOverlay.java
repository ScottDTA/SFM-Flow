 ...
 * canvas viewport [3]. Intercepts inputs and delegates them selectively to
 * active focused child targets [3].
 */
@OnlyIn(Dist.CLIENT)
public abstract class NodeSettingsOverlay extends AbstractFlowWidget {
	...
	// Clicked and focused element reference tracking [3]
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		...
		for (GuiEventListener child : children) {
			if (child.mouseClicked(mouseX, mouseY, button)) {
				this.focusedChild = child; // Track focused candidate to isolate subsequent drag sequences [3]
				return true;
			}
		}
		return false;
	...
	/**
	 * Isolates dragging actions exclusively to the active focused/clicked child
	 * [3].
	 */
	@Override
	...
	/**
	 * Releases active focused child tracking when the mouse button is released [3].
	 */
	@Override

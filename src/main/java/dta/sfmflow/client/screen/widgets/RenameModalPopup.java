...
public class RenameModalPopup extends AbstractModalPopup
 {
  ...
  public RenameModalPopup(ManagerScreen parentScreen, FlowWidgetContainer targetContainer)
   {
    ...
    this.nameEdit = new EditBox(parentScreen.getFont(), getX() + 5, getY() + 16, 100, 14, Component.literal("Rename"));
  ...
  public boolean mouseClicked(double mouseX, double mouseY, int button)
   {
    // 🔥 FIXED: Scan custom button coordinate bounds FIRST before passing to child widgets [3]
    // This stops parent bounds from pre-consuming the click and blocking the Save/Cancel buttons [3]
    int sx = getX() + 5;
    int sy = getY() + 36;
  ...
  protected void renderComponent(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick)
   {
    ...
    guiGraphics.drawString(parentScreen.getFont(), Component.literal("Rename Node"), getX() + 5, getY() + 4, 0xFF404040, false);

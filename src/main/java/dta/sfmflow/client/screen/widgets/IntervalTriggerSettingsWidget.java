...
/**
 * Custom settings panel widget dedicated to configuring the Interval Trigger
 * parameters [3]. Houses TimeUnit selection cycle buttons and range-clamped
 * delay duration slider controls [3].
 */
@OnlyIn(Dist.CLIENT)
public class IntervalTriggerSettingsWidget extends AbstractFlowWidget {
	...
	public IntervalTriggerSettingsWidget(FlowWidgetContainer container, IntervalTriggerComponent component) {
		super(container.getX(), container.getY(), component.getVisualWidth(), component.getVisualHeight(),
				Component.literal("Interval Settings"));
		this.container = container;
		this.component = component;

		final IntervalSlider[] sliderHolder = new IntervalSlider[1];

		// Instantiate TimeUnit CycleButton using displayOnlyValue() to clean up
		// formatting [3]
		...
		// Label directly above the TimeUnit CycleButton - configured with dynamic text
		// color [3]
		this.children.add(new FlowWidgetText(container.getParent().getFont(), getX() + 22, getY() + 25, 80, 10,
				Component.translatable("gui.sfmflow.time_unit"), 1.0F, true, () -> {
	...
	/**
	 * Standalone client-side Minecraft slider button subclass mapped to the
	 * Interval trigger settings [3].
	 */
	@OnlyIn(Dist.CLIENT)
	private static class IntervalSlider extends AbstractSliderButton {
		...
		public IntervalSlider(int x, int y, int width, int height, IntervalTriggerComponent component,
				FlowWidgetContainer container, IntervalTriggerSettingsWidget settingsWidget) {

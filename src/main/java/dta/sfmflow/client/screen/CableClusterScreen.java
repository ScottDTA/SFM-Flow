...
public class CableClusterScreen extends AbstractContainerScreen<CableClusterMenu> {
	 ...
	 */
	public CableClusterScreen(CableClusterMenu menu, net.minecraft.world.entity.player.Inventory playerInventory,
			Component title) {
		...
		if (numSlots == 9) {
			for (int i = 0; i < 9; i++) {
				...
				net.minecraft.client.gui.components.Button btn = net.minecraft.client.gui.components.Button
						.builder(Component.literal(name), b -> {
							...
							b.setMessage(
									Component.literal((nextDir == null) ? "NONE" : nextDir.name().substring(0, 2)));
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
		...
		if (numSlots == 9) {
			...
			for (int r = 0; r < 2; r++) {
				for (int c = 0; c < 9; c++) {
					guiGraphics.fill(x + 7 + c * 18, y + 17 + r * 34, x + 7 + c * 18 + 18, y + 17 + r * 34 + 18,
							0xFF151515);
		...
		for (int r = 0; r < 3; r++) {
			for (int c = 0; c < 9; c++) {
				guiGraphics.fill(x + 7 + c * 18, y + startY + r * 18, x + 7 + c * 18 + 18, y + startY + r * 18 + 18,
						0xFF151515);

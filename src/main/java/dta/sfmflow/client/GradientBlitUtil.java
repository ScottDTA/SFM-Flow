...
/**
 * Client-only graphic rendering utility for custom textured quads featuring
 * vertical color gradients [3].
 */
@OnlyIn(Dist.CLIENT)
public class GradientBlitUtil {
	...
	/**
	 * Safe vertex-colored blit utility that renders a vertical color gradient
	 * overlay onto a GUI texture [3]. Utilizes the single-L Tesselator and
	 * BufferBuilder under the position-texture-color shader configuration [3].
	 *
	 ...
	 * @param colorMask the custom Color mask applied to the vertices (can be null
	 *                  for white) [3]
	 */
	public static void blitWithGradient(Matrix4f matrix, ResourceLocation texture, int x, int y, int width, int height,
		...
		// Begin Tesselator and BufferBuilder under properties specified for
		// POSITION_TEX_COLOR [3]
		Tesselator tesselator = Tesselator.getInstance(); // 🔥 FIXED: Spelled Tesselator with single 'l' to match
															// Mojang mappings
		BufferBuilder bufferBuilder = tesselator.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_TEX_COLOR);

		// Quad Vertex 1 (Top Left)
		bufferBuilder.addVertex(matrix, (float) x, (float) y, 0.0F).setUv(minU, minV).setColor(topR, topG, topB, a);

		// Quad Vertex 2 (Bottom Left)
		bufferBuilder.addVertex(matrix, (float) x, (float) (y + height), 0.0F).setUv(minU, maxV).setColor(botR, botG,
				botB, a);
		...
		bufferBuilder.addVertex(matrix, (float) (x + width), (float) y, 0.0F).setUv(maxU, minV).setColor(topR, topG,
				topB, a);
	...
	public static float[] getBottomColorComponents(@javax.annotation.Nullable Color colorMask) {
		...
		return new float[] { Math.min(1.0F, mixedR * 0.75F), Math.min(1.0F, mixedG * 0.75F),

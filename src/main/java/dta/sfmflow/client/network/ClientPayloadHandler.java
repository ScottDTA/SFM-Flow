...
/**
 * Handles clientbound polymorphic delta synchronization payloads safely on the client thread.
 */
@OnlyIn(Dist.CLIENT)
public class ClientPayloadHandler {
    ...
    /**
     * Processes a S2C delta synchronization payload on the client thread.
     * Safely isolates world-load null states to prevent Render thread crashes.
     *
    ...
    public static void handleSyncComponentDelta(final SyncComponentDeltaPacket data, final IPayloadContext context) {
        // Enqueue the packet logic onto Minecraft's main drawing and ticks thread safely
        context.enqueueWork(() -> {
            Minecraft mc = Minecraft.getInstance();
            
            // 🔥 THE INPUT SHIELD: Stop processing immediately if the client world or player object is not ready yet!
            ...
            if (mc.screen instanceof dta.sfmflow.client.screen.ManagerScreen screen) {
                ...
                if (screen.getMenu() != null) {
                    ...
                    // Safe guard: ensure the underlying block entity reference is active on the client

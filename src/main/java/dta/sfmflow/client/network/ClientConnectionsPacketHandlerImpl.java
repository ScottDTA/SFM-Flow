            ...
            if (mc.screen instanceof dta.sfmflow.client.screen.ManagerScreen screen) {
                if (screen.getMenu() != null) {
                    ...
                    if (be != null && be.getBlockPos().equals(payload.pos())) {
                        ...
                        net.minecraft.nbt.ListTag listTag = payload.data().getList("connections", net.minecraft.nbt.Tag.TAG_COMPOUND);
                        for (int i = 0; i < listTag.size(); i++) {
                            connectionsList.add(dta.sfmflow.flowcomponents.FlowComponentConnections.load(listTag.getCompound(i)));
                        }

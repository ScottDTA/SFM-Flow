 ...
 * click tasks, and workspace state updates onto the main server thread sequential queue.
 */
public class ServerPayloadHandler
 {
  ...
  /**
   * Processes a client request to perform a canvas node action on the server thread [3].
   *
  ...
  /**
   * Processes a request to dynamically spawn a new node on the server thread, with safety checks [3].
   *
  ...
  /**
   * Processes a client drag layout modification payload on the main server thread [3].
   */
  ...
  public static void handleSaveComponentSettings(final SaveComponentSettings data, final IPayloadContext context)
   {
	context.enqueueWork(() -> {
	                           if (context.player().level().getBlockEntity(data.pos()) instanceof ManagerBlockEntity manager)
	                            {
                                 dta.sfmflow.api.component.AbstractFlowComponent component = manager.getFlowComponents().get(data.componentId());
                                 if (component != null)
                                  {
                                   component.loadData(data.settings());
                                   manager.setChanged();

                                   manager.broadcastDeltaUpdate(new SyncComponentDeltaPacket(
                                       manager.getBlockPos(),
                                       data.componentId(),
                                       SyncComponentDeltaPacket.DeltaType.SETTINGS,
                                       data.settings()
                                   ));
                                  }
	                            }
   ...
   */
  public static void handleSyncClusterSlotDirection(final SyncClusterSlotDirectionPacket data, final IPayloadContext context) {
      context.enqueueWork(() -> {
  ...
  /**
   * Processes an incoming connection request on the server main thread.
   * Enforces 1-wire limits on both pins and broadcasts connections sync packet [3].
   */
  public static void handleCreateConnection(final dta.sfmflow.networking.packets.serverbound.CreateConnectionPacket data, final IPayloadContext context) {
      context.enqueueWork(() -> {
          if (context.player().level().getBlockEntity(data.pos()) instanceof ManagerBlockEntity manager) {
              var connections = manager.getFlowConnections();
              
              // 1-wire limit check: Remove any old connection using either the same output or same input [3]
              connections.removeIf(conn -> 
                  (conn.getSourceComponentId().equals(data.sourceId()) && conn.getOutputNodeIndex() == data.outputIdx()) ||
                  (conn.getTargetComponentId().equals(data.targetId()) && conn.getInputNodeIndex() == data.inputIdx())
              );

              // Register new connection wire
              connections.add(new dta.sfmflow.flowcomponents.FlowComponentConnections(
                  data.sourceId(), data.outputIdx(), data.targetId(), data.inputIdx()
              ));
              manager.setChanged();
   ...
   * Cleans up the flowchart's wire array and broadcasts synchronization packets [3].
   */
  public static void handleRemoveConnection(final dta.sfmflow.networking.packets.serverbound.RemoveConnectionPacket data, final IPayloadContext context) {
      context.enqueueWork(() -> {
          if (context.player().level().getBlockEntity(data.pos()) instanceof ManagerBlockEntity manager) {
              var connections = manager.getFlowConnections();
              ...
              connections.removeIf(conn -> 
                  conn.getSourceComponentId().equals(data.sourceId()) && 

...
/**
 * Custom block model provider that generates single-face models for multi-sided
 * network blocks [3]. Utilizes direct raw JSON mutations to output explicit
 * scissor-matched cullface mappings [3].
 */
public class ModBlockModelProvider extends BlockModelProvider {
	...
	protected void registerModels() {
		...
		registerSingleFaceModel("redstone_emitter_face_off", "block/redstone_emitter_side_off",
				"block/redstone_emitter_face_off");
		registerSingleFaceModel("redstone_emitter_face_on", "block/redstone_emitter_side_on",
				"block/redstone_emitter_face_on");
	}

	/**
	 * Programmatically constructs a single culled-face model using raw JSON
	 * elements array injections [3].
	 *
	...
	private void registerSingleFaceModel(String modelName, String sideTexturePath, String faceTexturePath) {
		...
		// Define parent template cleanly using modern non-deprecated ResourceLocations
		// [3]
		var builder = withExistingParent(modelName, ResourceLocation.withDefaultNamespace("block/block"))
				.texture("particle", sideTexture).texture("face", faceTexture);
